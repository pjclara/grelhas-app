...
export default function CriteriosPage({ params }: { params: { turmaId: string } }) {
  ...
  async function adicionar(e: React.FormEvent) {
    ...
    await fetch(`/api/turmas/${params.turmaId}/criterios`, {
      method: 'POST',
  ...
  async function atualizarPeso(criterio: Criterio, novoPesoPct: string) {
    ...
    await fetch(`/api/turmas/${params.turmaId}/criterios/${criterio.id}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
  ...
  async function remover(id: string) {
    if (!confirm('Remover este critério? Os instrumentos associados ficam sem critério e devem ser reatribuídos.'))
      return;
    await fetch(`/api/turmas/${params.turmaId}/criterios/${id}`, { method: 'DELETE' });
    carregar();
  }
